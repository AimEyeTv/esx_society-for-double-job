...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if xPlayer.job.name == society or xPlayer.job2.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			xPlayer.addMoney(amount)
			xPlayer.showNotification("Hai pulito $" ..amount)
		else
			xPlayer.showNotification(_U('invalid_amount'))
		end
	else
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBossJob = xPlayer.job.grade_name == 'boss' and xPlayer.job.name == job or xPlayer.job.grade_name == 'boss' and 'unemployed' == job
	local isBossJob2 = xPlayer.job2.grade_name == 'boss' and xPlayer.job2.name == job or xPlayer.job2.grade_name == 'boss' and 'unemployed2' == job
	...
	if isBossJob2 then
		...
		if xTarget then
			...
			if type == 'hire' then
				xTarget.showNotification(_U('you_have_been_hired', xTarget.getJob2().label))
			elseif type == 'promote' then
				xTarget.showNotification(_U('you_have_been_promoted'))
			elseif type == 'fire' then
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	MySQL.Async.execute('UPDATE job_grades SET salary = @salary WHERE job_name = @job_name AND grade = @grade',
	{
		...
		for i=1, #xPlayers, 1 do
			...
			if xPlayer.job2.name == job and xPlayer.job2.grade == grade then
